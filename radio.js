...
try{
     var reg="/"+data.radio+"(.+)/i" ; var rgxp = eval(reg) ; var temp = ClaraIA.reco.match(rgxp) ; console.log(temp)
     var radio=temp[1].trim()
}catch(err){console.log('rien en 1, erreur de reco !');ClaraIARadio('');return}
...
request({'uri':'http://fluxradios.blogspot.com/2014/07/flux-url-'+radio+'.html', 'headers':{'Accept-Charset': 'UTF-8'},'encoding':'binary' },  (error, response, html) =>{
  ...
  catch(err){console.log('rien en 2, on coupe par defaut');ClaraIARadio('');return}  
})
return   
}